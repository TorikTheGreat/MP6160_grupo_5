...
typedef struct {
    ...
    XRgb2gray_top_uio_map maps[ MAX_UIO_MAPS ];
} XRgb2gray_top_uio_info;

/***************** Variable Definitions **************************************/
static XRgb2gray_top_uio_info uio_info;
...
static int uio_info_read_name(XRgb2gray_top_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XRgb2gray_top_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XRgb2gray_top_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XRgb2gray_top_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XRgb2gray_top_Initialize(XRgb2gray_top *InstancePtr, const char* InstanceName) {
	XRgb2gray_top_uio_info *InfoPtr = &uio_info;
