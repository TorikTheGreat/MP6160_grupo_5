...
baseline for evaluating hardware cost in terms of area and speed, they are unfeasible for strictly 
...
for raw Bayer images implemented on FPGAs \cite{regorsek2024} and structural optimizations a
pplied to the JPEG-LS standard under advanced coding architectures \cite{alonso2021} can sustain
...
such as the WHT \cite{alonso2021}. This omission makes them highly viable for general-purpose 
compression at the edge, but unfeasible if the architectural design goal is to maximize datapath
efficiency by exploiting a multiplier-free transformation core composed exclusively of $+1$ and $-1$
coefficients \cite{hafizullah2018}.
...
mapping in hardware for embedded systems \cite{jung1997}. Thus, although they are completely viable as
the numerical reference model for the system, they are unfeasible for direct physical deployment 
without a profound process of architectural exploration and synthesis.
...
\subsubsection{Memory Interface (AXI/TLM)}

This module manages data transfers between the accelerator and the external memory subsystem. The use of AXI/TLM interfaces facilitates integration into FPGA-SoC platforms such as the Kria KV260 and Alveo U55C, allowing the accelerator to be incorporated into larger embedded processing systems.
...
The proposed architecture will be compared against integer DCT implementations using hardware resource metrics (LUT/FF/DSP), justifying the WHT on hardware efficiency grounds.
\\ \hline

D4 -- Real-Time Throughput &
A fully pipelined datapath is proposed to support real-time video processing, sustaining one block per $N$ clock cycles.
\\ \hline

D5 -- Bit-Exact Lossless Reconstruction &
The reversible lifting-based formulation guarantees exact reconstruction. A Python/C reference model will be used to perform sample-by-sample comparison against hardware output.
\\ \hline
...
This section reports both functional behavior and hardware implementation
evidence for the proposed $N=8$ lossless WHT core. In addition to entropy and
bit-exactness checks, we include post-route FPGA metrics like critical datapath delay, implemented maximum frequency, timing slack, and
resource utilization.

\subsection{Experimental Setup}
A bit-accurate reference model of the transform was implemented in SystemC and
used as the verification oracle. The model reproduces the integer lifting
butterfly of the HLS core and adds the inverse transform, which the
synthesizable core does not implement. All experiments are reproducible from the
public repository \cite{repo_grupo5}: the reference model, the test benches, and
the datasets are generated deterministically from a fixed seed. The evaluation
uses five 8-bit grayscale $256\times256$ images spanning the range of spatial
correlation: a constant field, a linear ramp, a piecewise-constant image with
sharp edges, a $1/f$ surface with spatial correlation similar to natural images,
and white noise as a control. The transform is applied over one-dimensional
$N=8$ strips, matching the hardware; a separable two-dimensional variant
(row then column) is also evaluated as a software-only extension.
...
the Vitis HLS flow followed by post-route analysis in Vivado for device
\texttt{xcvc1902-vsva2197-2MP-e-S}. The design meets timing with positive slack
...
To validate the proposed architecture, a functional baseline implementation of the
forward Walsh--Hadamard Transform (FWHT) was developed in C++. This reference
implementation serves as the golden model for future hardware verification and
ensures that the arithmetic operations of the lifting-based transform are
correct before synthesis.




Table II summarizes the verification results obtained from the C++ baseline implementation.
\begin{table}[h]
\centering
...
These values were compared against the expected reference output and the test
bench reported a successful execution (PASS), confirming the correctness of the
baseline implementation.
The execution log generated during verification is available in the project repository.

As next step, these implemented metrics will be contrasted against
...
\bibitem{repo_grupo5} MP-6160 Grupo 5, ``WHT Lossless Accelerator Repository.'' [Online]. Available: https://github.com/MP6160-Grupo-5/MP6160\_grupo\_5
\end{thebibliography}
