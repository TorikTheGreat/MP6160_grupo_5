...
This stage performs the integer and reversible Walsh-Hadamard Transform. The implementation follows a multiplier-free approach, using only additions, subtractions, and arithmetic right shifts. This significantly reduces hardware resource utilization because dedicated DSP blocks are not required for multiplication operations. Instead, the implementation relies mainly on LUTs and Flip-Flops available within the target FPGA.

Bit-exact reconstruction is achieved through an integer lifting-based approximation \cite{jung1997}, enabling a multiplier-free implementation while guaranteeing lossless operation.
