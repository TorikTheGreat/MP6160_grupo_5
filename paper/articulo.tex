...
implemented using Vitis HLS and evaluated against a bit-accurate reference
model. Source-level functional verification showed exact agreement between the
reference model and the synthesizable forward and inverse cores over more than
100,000 random blocks each, and exact reconstruction through the cascaded
forward and inverse cores was confirmed over 400,000 cases; no C/RTL
cosimulation was performed. For
...
bit-accurate reference model was developed and used as the verification
...
To maximize datapath throughput for real-time video, the transform operations for a block size of $N=8$ were flattened into three combinational butterfly stages. The reported processing latency of 150 cycles is not due to the combinational datapath itself, but rather arises from the \texttt{m\_axi} memory transactions required to read and write the data bursts. We enforce maximal hardware concurrency by applying the \texttt{\#pragma HLS PIPELINE II=1} and \texttt{\#pragma HLS UNROLL} directives. Additionally, variables were strictly constrained using HLS arbitrary precision integers (\texttt{ap\_int<16>}), which prevents the datapath from unnecessarily routing 32-bit standard C++ integer bounds, ensuring a highly resource-efficient mapping on the Kria KV260 architecture. 

Although aggressive loop unrolling and full spatial parallelism inherently increase the combinatorial logic footprint, these datapath design decisions are rigorously justified by the strict latency requirements of real-time video processing. By prioritizing throughput, the core achieves an initiation interval of 1, allowing it to process a new data block every clock cycle. The resulting hardware cost (approximately 2400 LUTs) is a worthwhile trade-off, as it completely avoids the instantiation of DSP slices while comfortably meeting the timing and throughput constraints. The complete HLS source code for the transform core is publicly available in our repository \cite{repo_grupo5}.
...
The reversible lifting-based formulation guarantees exact reconstruction. A bit-accurate reference model performs sample-by-sample comparisons against the forward and inverse HLS cores at the C/C++ source level; verification against the generated RTL is not covered and remains as future work.
\\ \hline
...
A bit-accurate reference model of the transform was implemented in C++ with a
SystemC~2.3.4 signal-level wrapper (clock and \texttt{start}/\texttt{done}
handshake), compiled with g++~13.3.0. The model reproduces
the integer lifting butterfly of both synthesizable cores, forward and inverse.
The register-transfer implementation 
...
Three properties were checked, and we state for each one what it does and does
not cover.

First, \emph{forward equivalence} between the reference model and the
synthesizable core: over a fixed test vector and $10^5$ random 8-bit blocks,
both produce identical coefficients, with no divergence. Second,
\emph{inverse equivalence}: the reference inverse and the synthesizable inverse
core agree over a fixed vector and $10^5$ random coefficient vectors drawn from
the full signed 16-bit range, a wider stimulus than the forward test uses.
Both checks link the synthesizable sources directly, so each core is exercised
against an independent reference over its own input range. Third, \emph{losslessness}: cascading the two synthesizable cores recovers the
input exactly over $4\times10^5$ blocks, including the full signed 16-bit range.
This check is separate from the equivalence tests and necessary: the forward
core is exercised on 8-bit inputs, whose coefficients occupy a small region of
the 16-bit space, while the inverse core is exercised on uniform 16-bit vectors,
so neither equivalence test covers the composition. An equivalent check on the
reference model confirms that the lifting construction itself is reversible.

Regarding dynamic range, an 8-bit input cannot overflow the 16-bit datapath:
exhaustive evaluation over the extreme inputs $\{0,255\}^8$ gives a largest
coefficient magnitude of $1020$, well inside the range of a signed 16-bit word.
The same bound holds for $N=16$ ($2040$), so the datapath width is not a
constraint on larger block sizes.

Two limits of this verification should be stated explicitly. It is
\emph{source-level} equivalence, obtained by compiling the synthesizable sources
with a host compiler: no C/RTL cosimulation was performed, so the generated
register-transfer description itself is not covered. And the reference model and
the core implement the lifting butterfly with the same two expressions, which
means these checks detect indexing and stage-ordering errors --- as the
inverse-equivalence check does by construction --- but would not detect an error
in the shared formula. The SystemC wrapper drives the same reference functions
through a clocked interface and exercises $2001$ cases; the large-sample
evidence above comes from the C++ test benches.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.46\textwidth]{wht_reconstruction.png}}
\caption{Energy compaction and exact reconstruction on the natural-like image,
using the same one-dimensional $N{=}8$ transform that the synthesized core
implements. (b) regroups the coefficients by frequency band into eight vertical
strips on a square-root magnitude scale: the leftmost strip is the DC band and
retains a recognizable, horizontally compressed version of the image, while the
seven detail bands are close to zero. That concentration is what the entropy
reduction of Table~\ref{tab:entropy} measures. (d) is identically zero, which is
...
for the separable two-dimensional transform; in absolute terms the second
dimension adds $0.50$ bits/symbol on top of the $1.50$ that the
one-dimensional transform already recovers. Fig.~\ref{fig:reconstruction}
shows both effects on that image. White noise, having no spatial
...
resource comparisons. This baseline is independent of the bit-accurate golden
model used to verify the reversible lifting-based HLS core.
...
%
% ---------------------------------------------------------------------
% BORRADOR DE LIMITACIONES (W3) — listo para insertar dentro de esta
% sección cuando se escriba. Cubre el indicador de rúbrica "se identifican
% limitaciones en la generalización del diseño" (5 pts).
%
% No se inserta ya por dos motivos: (a) la sección que lo contiene todavía
% no existe, y (b) el cuerpo del artículo llega a la página 6, que es el
% límite del enunciado sin contar referencias. Si hay que recortar, las
% cuatro primeras son las que un tribunal pregunta.
%
% Cada afirmación está comprobada contra el repo; el detalle y la
% justificación punto por punto están en proyecto/defensa-w3-limitaciones.md
% ---------------------------------------------------------------------
%
% \subsection{Limitations}
% The scope of these results should be read with the following limits in mind.
% The synthesized core is fixed at $N=8$: the block size is a compile-time
% constant, so the design-space exploration reported here is over the reference
% model, not over synthesized variants. The core is one-dimensional; the
% separable two-dimensional results are a software extension of the model, and
% they recover an additional $0.50$ bits/symbol that the hardware does not
% deliver. Verification is at the C/C++ source level: the forward and inverse
% cores are compared against the reference model, but the generated RTL is not
% covered by cosimulation, and the reference model and the cores share the same
% lifting expressions, so an error in that shared formula would not be detected.
% All hardware figures come from a single device and a single family, and
% from post-route reports only: no measurements on a physical board were taken,
% and power was not estimated. Finally, the five images are synthetic and
% generated from a fixed seed; the ``natural'' case is a $1/f$ surface with
% image-like spatial correlation, not a camera frame, so the $20\%$ per-band
% reduction should be read as representative of that correlation structure rather
% than as a measurement on real video.
%
% OJO al insertarla: la limitación "forward-only" ya NO aplica (el núcleo
% inverso existe), pero articulo.tex:350 todavía dice lo contrario. Las dos
% cosas hay que arreglarlas juntas, y en ese orden importa: ver el aviso 4
% en proyecto/avisos-grupo.md.
