...
SystemC~2.3.4 transaction-level wrapper, compiled with g++~13.3.0, and used as the
verification oracle. The model reproduces
...
Both checks link the synthesizable sources directly, so a defect in either core
would be caught. Third, \emph{losslessness}: cascading the two synthesizable cores recovers the
...
in the shared formula. The SystemC wrapper is a transaction-level driver around
the same reference functions and exercises $2001$ cases; the large-sample
...
% The reversibility check over $4\times10^5$ cases runs on the reference model
% alone. All hardware figures come from a single device and a single family, and
