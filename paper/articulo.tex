...
over its own input range. Third, \emph{losslessness}: cascading the two synthesizable cores recovers the
...
The corresponding bound for $N=16$ is $2040$, so the datapath width is not a
...
constant, and the block-size sweep reported in this work was carried out on the
reference model, not over synthesized variants. The core is one-dimensional;
the separable two-dimensional results are a software extension of the model,
and they recover an additional $0.50$ bits/symbol that the hardware does not
...
All hardware figures come from a single device and a single family, and from
post-route reports only: no measurements on a physical board were taken, and
power was not estimated. Finally, the five images are synthetic and generated
from a fixed seed; the ``natural'' case is a $1/f$ surface with image-like
spatial correlation, not a camera frame, so the $20\%$ per-band reduction
should be read as representative of that correlation structure rather than as a
measurement on real video.

% Nota para quien escriba las conclusiones: esta seccion puede fundirse dentro
% de "Conclusions and Future Work" si se prefiere; se dejo aparte porque esa
% seccion todavia no existe. El detalle y la justificacion punto por punto de
% cada limitacion estan documentados fuera del repo.
