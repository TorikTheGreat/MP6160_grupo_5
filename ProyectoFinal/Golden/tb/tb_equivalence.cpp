...
// 
//  Compila el núcleo REAL de hardware (con g++ + shim ap_int) y lo compara,
//  coeficiente por coeficiente, contra el golden. N=8 (el núcleo HLS
...
int main() {
    ...
    // 1) El bloque fijo del testbench HLS (referencia ya validada a mano).
    ...
    std::cout << "E4 equivalencia golden<->nucleo HLS: "
              << (total - fails) << "/" << total
              << (fails ? "  FALLA\n" : "  PASA\n");
    return fails ? 1 : 0;
}
